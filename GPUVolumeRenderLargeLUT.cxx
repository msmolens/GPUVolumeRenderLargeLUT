...
int main(int argc, char *argv[])
{
  if (argc < 2)
    {
    std::cerr << "USAGE: " << argv[0] << " <filename.vti>" << std::endl;
    return EXIT_FAILURE;
    }
  ...
  vtkSmartPointer<vtkPiecewiseFunction> gradientOpacity =
    vtkSmartPointer<vtkPiecewiseFunction>::New();
  gradientOpacity->AddPoint(lut->GetRange()[0], 1.0);
  gradientOpacity->AddPoint(lut->GetRange()[1], 1.0);
...
#ifdef VTK_OPENGL2
  renderWindow->SetWindowName("OpenGL2");
#else
  renderWindow->SetWindowName("OpenGL");
#endif
  ...
  writer->SetFileName("baseline.png");
